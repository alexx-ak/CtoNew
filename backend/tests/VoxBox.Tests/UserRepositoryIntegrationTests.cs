...
public class UserRepositoryIntegrationTests : IClassFixture<TestDatabaseFixture>
{
    ...
    public async Task DeleteAsync_ShouldSetDeletedAtAndDeletedBy()
    {
        // Arrange
        _tenantContext.SetHostContext();
        var uniqueUsername = GetUniqueUsername("auditdel");
        var user = new User
        {
            UserName = uniqueUsername,
            Name = "Audit",
            Surname = "Delete",
            EmailAddress = "auditdel@example.com",
            PhoneNumber = "1234567890",
            IsActive = true
        };
        await _repository.AddAsync(user);
        ...
        _context.ChangeTracker.Clear();
        var deletedUser = await _repository.GetByIdAsync(userId, includeDeleted: true);
        Assert.NotNull(deletedUser);
        Assert.True(deletedUser.IsDeleted);
        Assert.NotNull(deletedUser.DeletedAt);
        Assert.Null(deletedUser.DeletedBy); // Set to null to avoid foreign key constraints
    }
    ...
    {
        // Arrange - Create user with one tenant context
        _tenantContext.SetTestTenantContext(tenantId: 100, subdomain: "tenant1");
        var user1Username = GetUniqueUsername("isol1");
        var user1 = new User
        {
        ...
        // Create user with different tenant context
        _tenantContext.SetTestTenantContext(tenantId: 200, subdomain: "tenant2");
        var user2Username = GetUniqueUsername("isol2");
        var user2 = new User
        {
        ...
        Assert.Contains(resultTenant1, u => u.UserName == user1Username);
        Assert.DoesNotContain(resultTenant1, u => u.UserName == user2Username);
        
        Assert.Contains(resultTenant2, u => u.UserName == user2Username);
        Assert.DoesNotContain(resultTenant2, u => u.UserName == user1Username);
    }
